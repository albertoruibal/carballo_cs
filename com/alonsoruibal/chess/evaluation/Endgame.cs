...
namespace Com.Alonsoruibal.Chess.Evaluation
{
	public class Endgame
	{
			...
			, int whiteQueens, int blackQueens)
		{
			...
			if (whitePawns == 0 && blackPawns == 0)
			{
				...
				if (whiteMaterial == 1 && blackMaterial == 1 && whiteRooks == 1 && blackRooks == 
					1)
				{
					return Endgame.EndgameKRKR(board);
				}
			}
			// Not always a draw
			...
			{
				//
				return Endgame.EndgameKXK(board, whiteMaterial > blackMaterial, whiteKnights + blackKnights
			...
			, int rooks, int queens)
		{
			...
			int value = Evaluator.KnownWin + knights * Evaluator.Knight + bishops * Evaluator
				.Bishop + rooks * Evaluator.Rook + queens * Evaluator.Queen + closerSquares[BitboardUtils
				.Distance(whiteKingIndex, blackKingIndex)] + (whiteDominant ? toCorners[blackKingIndex
				] : toCorners[whiteKingIndex]);
			//
		...
		private static int EndgameKPK(Board board, bool whiteDominant)
		{
			...
			return whiteDominant ? Evaluator.KnownWin + Evaluator.Pawn + BitboardUtils.GetRankOfIndex
				(BitboardUtils.Square2Index(board.pawns)) : -Evaluator.KnownWin - Evaluator.Pawn
				 - (7 - BitboardUtils.GetRankOfIndex(BitboardUtils.Square2Index(board.pawns)));
		}
